...
- **Syntax highlighting** for the full EPP grammar (versions 0.1 through
  0.4): `@commands`, quoted strings (with escape sequences and the
  `[newline]`/`[page]` placeholders highlighted distinctly), attribute
  blocks (`{align=center,bold}`), the v0.3 `@callout` command and its
  `color=` attribute, the `highlight=` attribute on `@text`/`@heading`,
  the page-level `{color=,lined,rotate=}` block on `@page`, the new
  v0.4 `@watermark`, `@sign`, and `@signandclose` commands, the new
  `@ex:*` extension namespace, comments, the version marker, and table
  cell separators.
- **Live preview**, just like Markdown's built-in preview: open a `.epp`
  ...
  scrolling past them. Page styling — background tints, notebook-style
  lined pages, rotated page content, watermarks, and signature lines —
  renders in the preview exactly as it would in the EPP Viewer browser
  extension. As in the reference renderer, `@signandclose` is a hard
  document terminator: content after it is not shown.
...
The complete EPP grammar — every command and attribute through v0.4,
including `@callout`, `highlight=`, the page-level `color=`/`lined`/
`rotate=` attributes, and the v0.4 `@watermark`/`@sign`/`@signandclose`/
`@ex:*` additions — is specified in `EPP_Frozen_Spec_v0_4.md` (shipped
