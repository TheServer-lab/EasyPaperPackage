...
function parseEPP(source){
  ...
  let watermark = null;
  let signAndClose = null;
  let signedAndClosed = false;
  const meta = {};
  const pages = [];
  ...
  while (true){
    ...
    if (cmd === 'watermark'){
      ...
      watermark = { text, attrs };
      continue;
    }
    // v0.4: @sign "NAME" {ORG-ID}  (page-scoped)
    if (cmd === 'sign'){
      skipWsAndComments();
      let name = '';
      if (source[i] === '"') name = readQuotedString();
      skipWsAndComments();
      let attrs = {};
      if (source[i] === '{') attrs = readAttrBlock();
      // Extract org ID — first key in attrs block (may be any type)
      const orgId = Object.keys(attrs)[0] || null;
      current.blocks.push({ type:'sign', name, orgId });
      continue;
    }
    // v0.4: @signandclose "NAME" {ORG-ID}  (document terminator)
    if (cmd === 'signandclose'){
      if (signedAndClosed) continue; // only first occurrence valid
      skipWsAndComments();
      let name = '';
      if (source[i] === '"') name = readQuotedString();
      skipWsAndComments();
      ...
      if (source[i] === '{') attrs = readAttrBlock();
      const orgId = Object.keys(attrs)[0] || null;
      signAndClose = { name, orgId };
      signedAndClosed = true;
      // Render in-page (identical to a normal @sign block) so it's part of
      // the page's own content and prints with it, instead of floating
      // outside .epp-pages.
      current.blocks.push({ type:'sign', name, orgId });
      pages.push(current); // push final page before stopping
      current = { number:null, blocks:[], pageAttrs:{} };
      break; // hard document terminator — stop parsing
    }
    // v0.4: @ex:anything — extension namespace, ignore gracefully
  ...
  if (!signedAndClosed) pages.push(current);
  return { version, title, meta, header, footer, watermark, signAndClose, pages };
}
...
function renderPageInner(page, pageNumber, totalPages, header, footer, doc_watermark){
  let html = '';
  // v0.4: watermark
  if (doc_watermark){
    const wt = doc_watermark;
    const wmColor = wt.attrs && wt.attrs.color && COLOR_MAP[wt.attrs.color] ? COLOR_MAP[wt.attrs.color] : '#5b564a';
    const wmOpacity = (wt.attrs && wt.attrs.opacity !== undefined) ? Math.min(100,Math.max(0,parseInt(wt.attrs.opacity)))/100 : 0.18;
    const wmRotate = (wt.attrs && wt.attrs.rotate) ? parseInt(wt.attrs.rotate) : 0;
    const wmSize = (wt.attrs && wt.attrs.size) ? Math.max(40,Math.min(300,parseInt(wt.attrs.size))) : 100;
    const wmFontSize = Math.round(wmSize * 0.7);
    html += `<div class="epp-watermark" style="color:${wmColor};opacity:${wmOpacity};transform:rotate(${wmRotate}deg);font-size:${wmFontSize}px">${escapeHtml(wt.text)}</div>`;
  }
  let openList = false;
  let listCounter = 0;
  ...
  const blocksHtml = (() => {
    ...
    page.blocks.forEach((block, idx) => {
      ...
      if (block.type === 'heading'){
        ...
        out += `<div class="epp-sign">—${escapeHtml(block.name)}</div>`;
      }
    });
    if (openList) out += '</ul>';
...
function renderDocumentHTML(doc){
  ...
  return doc.pages.map((page, pIdx) => {
    ...
    const linedStyle = pa.lined
      ? `background-image:repeating-linear-gradient(transparent,transparent 25px,rgba(185,175,152,.4) 26px);background-size:100% 26px;background-position:0 6px;`
      : '';
    ...
    return `<div class="epp-page${rotClass}${linedClass}" data-index="${pIdx}" style="${pageStyle}">` +
      `<div class="epp-page-inner">${renderPageInner(page, number, total, doc.header, doc.footer, doc.watermark)}</div>` +
    `</div>`;
  }).join('');
}
