...
(async () => {
  ...
  function showStatus(icon, msg, url, showRetry) {
    document.querySelector('#epp-status .status-icon').textContent = icon;
  ...
  if (!src) {
    // No ?src= param — show the built-in sample
    mountViewer(root, parseEPP(SAMPLE_SOURCE), { showOpenButton: true });
    return;
  }

  // A source URL was provided by background.js redirect
  async function load() {
    ...
    try {
      ...
      if (!/^\s*%epp=/.test(text)) throw new Error('Not a valid EPP document (missing %epp=\u2026% header).');
      hideStatus();
      mountViewer(root, parseEPP(text), { showOpenButton: true, sourceUrl: src });
    } catch (err) {
      showStatus('\u26A0\uFE0F', err.message || 'Failed to load file.', src, true);
    }
  }
